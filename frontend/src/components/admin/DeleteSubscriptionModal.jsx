...
export default function DeleteSubscriptionModal({ open, subscription, onClose, onSuccess }) {
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted">
          Delete the {subscription?.plan_name ?? "subscription"}
          {subscription?.user_first_name ? (
            <>
              {" "}
              for{" "}
              <span className="font-medium text-foreground">
                {subscription.user_first_name} {subscription.user_last_name}
              </span>
            </>
          ) : null}
