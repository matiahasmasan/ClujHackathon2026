    ...
    <main className="flex-1 space-y-6 p-4 sm:p-6">
      ...
      {!loading && !error && payments.length > 0 && (
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="rounded-2xl bg-card/75 p-5 shadow-sm">
            <p className="text-sm text-muted">Total payments</p>
            <p className="mt-1 text-2xl font-bold text-foreground">{stats.total}</p>
          </div>
          <div className="rounded-2xl bg-card/75 p-5 shadow-sm">
            <p className="text-sm text-muted">Successful</p>
            <p className="mt-1 text-2xl font-bold text-foreground">{stats.succeeded}</p>
          </div>
          <div className="rounded-2xl bg-card/75 p-5 shadow-sm">
            <p className="text-sm text-muted">Total paid</p>
            <p className="mt-1 text-2xl font-bold text-foreground">{stats.totalPaid}</p>
          </div>
        </div>
      )}
      ...
      {!loading && !error && payments.length > 0 && (
        <ul className="divide-y divide-border/50 rounded-2xl bg-card/75 shadow-sm backdrop-blur-sm">
          {payments.map((payment) => (
            <li key={payment.id} className="dashboard-row-hover p-5">
