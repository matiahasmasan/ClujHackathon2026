...
/**
 * Call POST /api/calls (requires Bearer token).
 */
export async function createCall(payload) {
  ...
  if (!response.ok) {
    throw new Error(parseErrorMessage(data, "Could not log call."));
  }
...
/**
 * Call PATCH /api/calls/:id (requires Bearer token).
 */
export async function updateCall(callId, payload) {
  let response;
  try {
    response = await fetch(`${API_URL}/calls/${callId}`, {
      method: "PATCH",
      headers: authHeaders(),
      body: JSON.stringify(payload),
    });
  ...
  if (!response.ok) {
    throw new Error(parseErrorMessage(data, "Could not update call."));
  }
...
/**
 * Call DELETE /api/calls/:id (requires Bearer token).
 */
export async function deleteCall(callId) {
  ...
  if (!response.ok) {
    throw new Error(parseErrorMessage(data, "Could not delete call."));
  }
}
